...
class IndicadorTareas:

    @staticmethod
    def _gradient_colors(values, thresholds=(0.30, 0.60, 0.80)):
        t1, t2, t3 = thresholds
        styles = []
        for v in values:
            if pd.isna(v):
    ...
    def aplicar_estilos(cls, df):
        ...
        return df.style.apply(cls._gradient_colors, subset=alcance_cols, axis=0).format(formato)

    @staticmethod
    def _agregar_equipo(df_tareas, df_usuarios):
        df_merge = df_tareas.merge(
            df_usuarios[["Codigo", "Supervisor", "Rep. Ventas"]],
            left_on="BDR ID",
            right_on="Codigo",
            how="left"
        ).drop(columns="BDR ID")
        return df_merge[df_merge["Supervisor"].notna()]

    @classmethod
    def calcular(cls, df_tareas, df_usuarios):
        tarea = cls._agregar_equipo(df_tareas, df_usuarios)
        ...
        return pd.concat([pivot_true, pivot_pct], axis=1).sort_index(axis=1)
