...
if "df_usuarios" in st.session_state and not st.session_state.df_usuarios.empty:
    st.success("Archivo de usuarios cargado")
    st.write(f"Filas: {st.session_state.df_usuarios.shape[0]} | Columnas: {st.session_state.df_usuarios.shape[1]}")
    st.dataframe(st.session_state.df_usuarios)
else:
    st.info("Aún no se ha cargado el archivo de usuarios")

def refresh_data(verbose: bool = False, limit: int | None = None) -> None:
    st.session_state["df_usuarios"] = leer_archivo()

st.button(
    "Actualizar datos",
    key="refresh",
    help="Actualiza la información de usuarios por supervisor",
    type="primary",
